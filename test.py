...
from taglookup import gettag, getsubtags
...
for field in readmarc(testrecord):
    print(gettag(field[0]))
    if '' in field[1]:
        subfields = parsesf(field[1])
        subtags = []
        subdata = []
        for entry in subfields:
            subtags.append(entry[0])
            subdata.append(entry[1])
        subtags = getsubtags(field[0], subtags)
        a = 0
        subfields = []
        for tag in subtags:
            pair = (tag, subdata[a])
            subfields.append(pair)
            a = a + 1
